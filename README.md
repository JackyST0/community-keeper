...
它不需要部署服务，不需要手动填写 Cookie，也不会把账号信息上传到第三方。你只需要在浏览器里正常登录对应社区，然后通过扩展弹窗手动执行任务。
...
这个项目目前主要面向本地使用和社区分发，不依赖 Chrome Web Store。

普通用户建议通过 GitHub Release 下载压缩包；维护者可以运行下面的脚本生成新的分发包：
